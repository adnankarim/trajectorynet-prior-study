...
# --------------------------------------------------------------------------
#  MSE v2  (Torch-only version, no 'ot' dependency)
# --------------------------------------------------------------------------


def evaluate_mse_v2(device, args, model, growth_model=None):
    """
    ...
    if args.use_growth or growth_model is not None:
        print("WARNING: growth_model is ignored for MSE v2")

    lo = args.leaveout_timepoint
    tp = args.int_tps                                   # shortcut

    # ---------- gather tensors -------------------------------------------------
    next_np = args.data.get_data()[args.data.get_times() == lo + 1]
    prev_np = args.data.get_data()[args.data.get_times() == lo - 1]
    true_np = args.data.get_data()[args.data.get_times() == lo]

    next_z = torch.tensor(next_np, dtype=torch.float32, device=device)
    prev_z = torch.tensor(prev_np, dtype=torch.float32, device=device)

    # ---------- integrate one hop ---------------------------------------------
    #  (a) backwards from t_{LO+1}
    int_times = torch.tensor([tp[lo], tp[lo + 1]], dtype=torch.float32,
                             device=device)
    pred_b, _ = model.chain[0](next_z, torch.zeros(next_z.size(0), 1,
                                                   device=device),
                               integration_times=int_times)
    pred_b = pred_b.detach()

    #  (b) forwards from t_{LO-1}
    int_times = torch.tensor([tp[lo - 1], tp[lo]], dtype=torch.float32,
                             device=device)
    pred_f, _ = model.chain[0](prev_z,
                               torch.zeros(prev_z.size(0), 1, device=device),
                               integration_times=int_times,
                               reverse=True)
    pred_f = pred_f.detach()

    # ---------- Chamfer-style symmetric MSE -----------------------------------
    true_t = torch.tensor(true_np, dtype=torch.float32, device=device)

    def _chamfer_mse(pred, true):
        # squared pairwise distances [N_pred, N_true]
        D = torch.cdist(pred, true, p=2.0) ** 2
        mse_pt = D.min(dim=1).values.mean()        # pred → true
        mse_tp = D.min(dim=0).values.mean()        # true → pred
        return 0.5 * (mse_pt + mse_tp)

    mse_b = _chamfer_mse(pred_b, true_t).item()
    mse_f = _chamfer_mse(pred_f, true_t).item()

    mses = np.array([mse_b, mse_f], dtype=np.float64)
    np.save(os.path.join(args.save, "mses_v2.npy"), mses)
    return mses
...
def evaluate_kantorovich_v2(device, args, model, growth_model=None):
    ...
    if args.use_growth or growth_model is not None:
        ...
        # )
        print("WARNING: Ignoring growth model and computing anyway")
    print(args.data.get_data().shape)
...
def evaluate_kantorovich(device, args, model, growth_model=None, n=10000):
    ...
    with torch.no_grad():
        ...
        if hasattr(args.data, "has_validation_samples") and args.data.has_validation_samples():

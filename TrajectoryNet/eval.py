...
def integrate_backwards(
    end_samples, model, savedir, ntimes=100, memory=0.1, device="cuda"
):
...
def main(args):
    ...
    # ---- Kantorovich v2 ---------------------------------------------------------
    try:
        results["Kantorovich v2"] = eval_utils.evaluate_kantorovich_v2(device, args, model)
        if isinstance(results["Kantorovich v2"], (np.ndarray, list)) and len(results["Kantorovich v2"]) > 0:
            results["Kantorovich v2 mean"] = float(np.mean(results["Kantorovich v2"]))
    except Exception as err:
        print("\n[WARNING] Kantorovich v2 failed → returning None")
        traceback.print_exc()
        results["Kantorovich v2"] = None
        results["Kantorovich v2 mean"] = None

    # ---- Kantorovich v1 ---------------------------------------------------------
    try:
        results["Kantorovich v1"] = eval_utils.evaluate_kantorovich(device, args, model)
        if isinstance(results["Kantorovich v1"], (np.ndarray, list)) and len(results["Kantorovich v1"]) > 0:
            results["Kantorovich v1 mean"] = float(np.mean(results["Kantorovich v1"]))
    except Exception as err:
        print("\n[WARNING] Kantorovich v1 failed → returning None")
        traceback.print_exc()
        results["Kantorovich v1"] = None
        results["Kantorovich v1 mean"] = None

    # ---- MSE --------------------------------------------------------------------
    try:
        results["MSE"] = eval_utils.evaluate_mse(device, args, model)
        if isinstance(results["MSE"], (np.ndarray, list)) and len(results["MSE"]) > 0:
            results["MSE mean"] = float(np.mean(results["MSE"]))
    except Exception as err:
        print("\n[WARNING] MSE evaluation failed → returning None")
        traceback.print_exc()
        results["MSE"] = None
        results["MSE mean"] = None
    # ---- MSE v2 (torch) --------------------------------------------------------
    try:
        results["MSE v2"] = eval_utils.evaluate_mse_v2(device, args, model)
        if len(results["MSE v2"]) > 0:
            results["MSE v2 mean"] = float(np.mean(results["MSE v2"]))
    except Exception:
        print("\n[WARNING] MSE v2 evaluation failed → returning None")
        traceback.print_exc()
        results["MSE v2"] = None
        results["MSE v2 mean"] = None

    # ---- Accumulate & save ------------------------------------------------------
    losses_list.append(list(results.values()))
    np.save(os.path.join(args.save, "emd_list.npy"), np.array(losses_list, dtype=object))

    # ---- Pretty print table -----------------------------------------------------
    def _format(v):
        return np.array2string(v, precision=4) if isinstance(v, np.ndarray) else str(v)

    table_rows = [[k, _format(v)] for k, v in results.items()]

    try:
        from tabulate import tabulate
        print("\n" + tabulate(table_rows, headers=["Metric", "Value"], tablefmt="github"))
    except ImportError:
        # fallback: crude column print
        print("\nMetric              | Value")
        print("--------------------|-------------------------------")
    ...
    # ---- Export to CSV ----------------------------------------------------------
    csv_path = os.path.join(args.save, "eval_results.csv")
...
# for d in tree_base tree_baseD tree_baseDV tree_base_V; do 
#   python -m TrajectoryNet.eval --dataset TREE --embedding_name PCA --leaveout_timepoint 1 --save results/$d; 
# done; 
...
# done
# python -m TrajectoryNet.eval --dataset TREE --embedding_name PCA --leaveout_timepoint 1 --save results/tree_base

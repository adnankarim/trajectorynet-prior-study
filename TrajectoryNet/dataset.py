...
# dataset_repro.py  – put next to dataset.py and import as needed
import numpy as np, torch, math, scipy.sparse
import ot                                                                # pip install POT

from sklearn.preprocessing import StandardScaler
from TrajectoryNet.mixins import PriorMixin
...
class SCData(object):
    ...
    def factory(name, args):
        ...
        if name == "ARCHPAPER":
            return ArchDatasetPaper()
        if name == "CIRCLE5":
            return CircleTestDataV5()
        if name == "TREE":
            return TreeTestData()
        if name == "TREEPAPER":
            return TreeTestDataPaper()
        if name == "CYCLE":
            return CycleDataset()
...
# Add this at the end of dataset.py
class TreeTestDataPaper:
    def __init__(self,
                 n_per_end=5000,
                 sigma_theta=0.5/np.pi,
                 sigma_r=0.10,
                 tangent_step=0.3,
                 reflect_side="right",
                 reflect_frac=0.5,
        ...
        z0 = np.abs(rng.normal(0, sigma_theta, size=n_per_end))
        z2 = 1.0 - np.abs(rng.normal(0, sigma_theta, size=n_per_end))
        data_1d = np.concatenate([z0, z2])
        labels = np.concatenate([np.zeros(n_per_end), np.full(n_per_end, 2)])
        import ot
        from TrajectoryNet.dataset import interpolate_with_ot
        gamma = ot.emd_1d(z0, z2)
        z1 = interpolate_with_ot(z0[:, None], z2[:, None], gamma, 0.5, n_per_end)
        data_1d = np.concatenate([data_1d, z1.flatten()])
        labels = np.concatenate([labels, np.full(n_per_end, 1)])
        theta = data_1d * np.pi
        r = 1.0 + rng.normal(0, sigma_r, size=len(theta))
        x2d = np.stack([np.cos(theta), np.sin(theta)], axis=1) * r[:, None]
        if reflect_side == "right":
            side_mask = x2d[:, 0] > 0
        else:
            side_mask = x2d[:, 0] < 0
        choose = rng.random(len(x2d)) < reflect_frac
        mask = side_mask & choose
        x2d[mask] = np.column_stack([x2d[mask, 0], 2.0 - x2d[mask, 1]])
        theta_next = theta + tangent_step
        next2d = np.stack([np.cos(theta_next), np.sin(theta_next)], axis=1) * r[:, None]
        velocity = next2d - x2d
        self.data = x2d.astype(np.float32)
        self.velocity = velocity.astype(np.float32)
        self.labels = labels.astype(np.int64)
        self.ncells = len(self.data)
    ...
    def sample_index(self, n, tp): return np.random.choice(np.where(self.labels==tp)[0], size=n, replace=False)
    def known_base_density(self): return True
    def base_density(self):
        def logp(z):
            logZ = -0.5 * math.log(2 * math.pi)
            return torch.sum(logZ - 0.5 * z.pow(2), 1, keepdim=True)
        return logp
...
class CycleDataset(TreeTestData):
    ...
    def __init__(self, shift=0.1, r_std=0.1):
        ...
        # Generate data for three time points
        data = np.random.rand(n)
        data = np.concatenate([data, data + shift, data + 2 * shift]) # Now has 15000 points
        
        # Generate radii for all points
        r = np.tile(np.ones(n) + np.random.randn(n) * self.r_std, 3)[:, np.newaxis] # Now has 15000 points
        
        # Generate three labels, one for each time point/shift
        self.labels = np.repeat(np.arange(3), n) # Changed from 2 to 3. Now has 15000 labels.
        print(f"labels shape: {self.labels.shape}") # Should show (15000,)

        # Convert to 2D coordinates
        theta = data * 2 * np.pi
        x2d = np.array([np.cos(theta), np.sin(theta)]).T * r # Now has 15000 points
        
        # Assign all 15000 points to self.data
        self.data = x2d # Changed from x2d[n:]
        
        # Keep old_data as the first 5000 points (optional, adjust if needed)
        self.old_data = x2d[:n] 
        
        # Calculate velocity based on a small shift for all points
        next_theta = theta + 2 * np.pi * shift * 0.001
        next2d = np.array([np.cos(next_theta), np.sin(next_theta)]).T * r
        
        # Calculate velocity for all 15000 points
        self.velocity = ((next2d - x2d) * 1000) * 2 # Removed [n:]
        
        # Update ncells to reflect the total number of points
        self.ncells = 3 * n # Changed from 2 * n
        print(f"Number of cells (ncells): {self.ncells}") # Should show 15000
        print(f"Data shape: {self.data.shape}") # Should show (15000, 2)

    # ... (rest of the class methods: get_paths, base_density, etc.) ...
    def get_paths(self, n=5000, n_steps=3):
        # Only 3 steps are supported at this time.
        assert n_steps == 3
        shift = self.shift

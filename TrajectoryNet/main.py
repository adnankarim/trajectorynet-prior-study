...
def compute_loss(device, args, model, growth_model, logger, full_data):
    ...
    for i, (itp, tp) in enumerate(zip(args.int_tps[::-1], args.timepoints[::-1])):
        ...
        # Print all shapes and values
        print("\n=== Shape Debug Info ===")
        print(f"x shape              : {x.shape}")
        print(f"zero shape           : {zero.shape}")
        if i > 0:
            print(f"z shape              : {z.shape}")
        print(f"integration_times     : {integration_times}")
        print(f"integration_times shape: {integration_times.shape}")
        print("========================\n")
        # exit()
    ...
    print(f"deltas[-1] shape {deltas[-1].shape}\n")
    print("========================\n")
    exit()
    # build growth rates
    if args.use_growth:
    ...
    if args.vecint:
        ...
        print("Direction Loss", similarity_loss.item())
...
def main(args):
    ...
    args.data.configure_prior(args)      # NEW  ← toggles Gaussian / GMM / NSF
